...
public class Main {
    public static void main(String[] args) {
        ...
        Contenido c4 = audiolibroF.crearContenido("A01", "El caballero de la armadura oxidada", 5040  , "Robert Fisher"," Ficcion");
        ...
        //DECORATOR y PROXY
        IReproductor paraMiltonMiguel = construirReproductor(miltonMiguel, true, true, false);
        IReproductor paraVictorHugo     = construirReproductor(victorHugo,     true, false, true);

        System.out.println("\n--- Milton Miguel (PREMIUM) reproduce un audiolibro ---");
        paraMiltonMiguel.reproducir(c4);

        System.out.println("\n--- Victor Hugo (FREE) intenta un audiolibro (bloqueado) ---");
        paraVictorHugo.reproducir(c4);

        System.out.println("\n--- Victor Hugo (FREE) escucha una canción (con anuncio) ---");
        paraVictorHugo.reproducir(c1);
    }
